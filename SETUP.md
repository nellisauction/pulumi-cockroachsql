...
From the templated repository run the init program and follow the prompts to replace the name "cockroachsql" with the name of your provider:
...
- `provider/` Go code used to create the provider as well as generate the SDKs in the various languages that Pulumi supports
  - `provider/cmd/pulumi-tfgen-cockroachsql` executable program to generate the Pulumi resource schema
  - `provider/cmd/pulumi-resource-cockroachsql` provider plugin executable
...
1. Copy the `pulumi-resource-cockroachsql` binary generated by `make provider` and place it in your `$PATH` (`$GOPATH/bin` is a convenient choice), e.g.:

    ```bash
    cp bin/pulumi-resource-cockroachsql $GOPATH/bin
    ```
...
1. Configure any necessary environment variables for authentication, e.g `$COCKROACHSQL_USERNAME`, `$COCKROACHSQL_TOKEN`, in your local environment.
...
Integration tests are Go tests residing in `examples/*_test.go`. Each test executes a program from a subdirectory within `examples/`. All tests are initially marked to be skipped as the example `cockroachsql` provider is not fully implemented and will fail with "not implemented".
...
1. **Add resource mappings:** For each resource in the provider, add an entry in the `Resources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "cockroachsql_something":      {Tok: tfbridge.MakeResource(mainPkg, mainMod, "Something")},
    "cockroachsql_something_else": {Tok: tfbridge.MakeResource(mainPkg, mainMod, "SomethingElse")},
    ```
...
1. **Add data source mappings:** For each data source in the provider, add an entry in the `DataSources` property of the `tfbridge.ProviderInfo`, e.g.:
    ...
    "cockroachsql_something":      {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomething")},
    "cockroachsql_something_else": {Tok: tfbridge.MakeDataSource(mainPkg, mainMod, "getSomethingElse")},
    ```
...
Pulumi's Terraform bridge automatically detects configuration options for the upstream provider.  However, in rare cases these settings may need to be overridden, e.g. if we want to change an environment variable default from `API_KEY` to `COCKROACHSQL_API_KEY`.  Examples of common uses cases:
...
"apikey": {
    Default: &tfbridge.DefaultInfo{
        EnvVars: []string{"COCKROACHSQL_API_KEY"},
    },
```
